...
export class MysqlRepository implements PagoReposotiry {
    ...
    createPago = async (pagos: Pagos): Promise<any> => {
        const sql = 'INSERT INTO pagos(nombre, apellido, cantidad, panecillo, telefono) VALUES (?, ?, ?, ?, ?)';
        const params = [pagos.nombre, pagos.apellido, pagos.cantidad, pagos.panecillo, pagos.telefono];
